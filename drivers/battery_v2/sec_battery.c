...
int sec_bat_set_charging_current(struct sec_battery_info *battery)
{
	...
	if (battery->cable_type != SB_CBL_NONE) {
		/* Needs to run always */
		if (battery->voltage_avg > battery->pdata->cc2_voltage_threshold) { /* 0.7C */
			sec_vote(battery->fcc_vote, VOTER_CSTEP, false, 0);
			sec_vote(battery->fcc_vote, VOTER_CC2_VOL, true, battery->pdata->cc2_charge_current);
		}
				...
				struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_THERMAL_SOURCE_ADC:
		...
		if (battery->pdata->temp_check_type == SEC_BATTERY_TEMP_CHECK_NONE)
			battery->temperature = 318; /* dummy value */
		else
			battery->temperature = value.intval;
...
void sec_bat_get_battery_info(struct sec_battery_info *battery)
{
	...
	secdbg_exin_set_batt(battery->capacity, battery->voltage_now, battery->temperature, battery->current_now);
#endif
}
...
static int sec_bat_parse_chg_current_dt(struct device_node *np,
		sec_charging_current_t *chg_curr, unsigned int def_icl, unsigned int def_fcc, unsigned int def_ttf_fcc)
{
	struct bat_temp_limit *btl = NULL;
	struct device_node *child;
	unsigned int i, icl, fcc, temp, ttf_fcc;
	int len = 0;

	if (np == NULL || chg_curr == NULL)
		return -EINVAL;
	...
	for (i = 0; i <= len; i++) {
		...
		chg_curr[temp].input_current_limit = icl;
		chg_curr[temp].fast_charging_current = fcc;
		chg_curr[temp].ttf_fcc = ttf_fcc;
		chg_curr[temp].btl = btl;
		pr_info("%s: [%d] set ext chg (bat_temp_limit = %s)\n",
			__func__, temp, GET_BOOL_STR(btl != NULL));
	}
		...
		struct sec_battery_info *battery)
{
	...
	if (!np) {
		...
		struct device_node *child;
		u32 input_current = 0, charging_current = 0, ttf_fcc = 0;
		...
		pdata->charging_current =
			kzalloc(sizeof(sec_charging_current_t) * SB_CBL_MAX,
				GFP_KERNEL);
		...
		for_each_child_of_node(np, child) {
			ret = sec_bat_parse_chg_current_dt(child,
				pdata->charging_current, input_current, charging_current, ttf_fcc);
			if (ret != 0)
	...
	ret = of_property_read_u32(np, "battery,cc2_voltage_threshold",
				&pdata->cc2_voltage_threshold);
	if (ret) {
		pr_info("%s: cc2_voltage_threshold is Empty\n", __func__);
		pdata->cc2_voltage_threshold = pdata->chg_float_voltage;
	}

	ret = of_property_read_u32(np, "battery,cc2_charge_current",
				&pdata->cc2_charge_current);
	if (ret) {
		pr_info("%s: cc2_charge_current is Empty\n", __func__);
		pdata->cc2_charge_current = pdata->default_charging_current;
	}
...
static int sec_bat_parse_dt_for_factory(struct sec_battery_info *battery)
{
	...
	if (ret) {
		icl = pdata->default_input_current;
	...
	if (ret) {
		fcc = pdata->default_charging_current;
	...
	ret = of_property_read_u32(np, "battery,factory_wire_normal_warm_thresh",
		(unsigned int *)&temp);
	if (ret) {
		temp = battery->pdata->wire_warm_overheat_thresh - 10;
		pr_info("%s: factory_wire_normal_warm_thresh is Empty, set default value(%d)\n",
			__func__, temp);
	}
	battery->pdata->wire_normal_warm_thresh = temp;
	battery->pdata->wireless_normal_warm_thresh = temp;
