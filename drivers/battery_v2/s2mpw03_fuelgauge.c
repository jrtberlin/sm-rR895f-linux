...
static int s2mpw03_get_vbat(struct s2mpw03_fuelgauge_data *fuelgauge, bool oldval_update);
...
static int s2mpw03_get_soc(struct s2mpw03_fuelgauge_data *fuelgauge)
{
	...
	if (!fuelgauge->factory_mode && (ocv < 3000 || ocv > 4500 || rs < 300 || vts > 300 || vts < -300 ||
			vtl > 300 || vtl < -300 || vdiff < -1500 || vdiff > 1500)){
	...
	vbat_now = s2mpw03_get_vbat(fuelgauge, true);
	...
	if (rsoc_old != 0 && vbat_old != 0 && (abs(rsoc_old - rsoc) > 500)
...
static int s2mpw03_get_vbat(struct s2mpw03_fuelgauge_data *fuelgauge, bool oldval_update)
{
	...
	if ((fuelgauge->voltage_old == compliment) && !fuelgauge->factory_mode) {
		...
		if (fuelgauge->v_check_cnt >= fuelgauge->pdata->v_check) {
			pr_info(" : voltage stuck detected!");
...
static int s2mpw03_fg_get_scaled_capacity_max(struct s2mpw03_fuelgauge_data *fuelgauge)
{
	int ret;
	u8 val;

	ret = s2mpw03_read_reg(fuelgauge->rtc, S2MP_RTC_REG_CTRL, &val);
	if (ret < 0) {
...
static int s2mpw03_fg_set_scaled_capacity_max(struct s2mpw03_fuelgauge_data *fuelgauge)
{
	u8 val;
	int scale_factor = 0;
	int ret = 0;
	...
	ret = s2mpw03_update_reg(fuelgauge->rtc, S2MP_RTC_REG_CTRL,
	...
	s2mpw03_read_reg(fuelgauge->rtc, S2MP_RTC_REG_CTRL, &val);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = s2mpw03_get_vbat(fuelgauge, false);
		break;
		/* Additional Voltage Information (mV) */
	case POWER_SUPPLY_PROP_VOLTAGE_AVG:
		switch (val->intval) {
		case SEC_BATTERY_VOLTAGE_AVERAGE:
			val->intval = s2mpw03_get_avg_val(&fuelgauge->avg_vol, s2mpw03_get_vbat(fuelgauge, false));
			if (!val->intval)
				val->intval = s2mpw03_get_vbat(fuelgauge, false);
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		if (fuelgauge->pdata->capacity_calculation_type &
			...
			s2mpw03_fg_set_scaled_capacity_max(fuelgauge);
		}
		break;
...
static int s2mpw03_fuelgauge_probe(struct platform_device *pdev)
{
	...
	s2mpw03_fg_get_scaled_capacity_max(fuelgauge);
